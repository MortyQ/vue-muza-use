...
const { data, execute } = useApi<Summary>("/analytics/summary", {
    immediate: true,
...
const code = `const { data } = useApi<Summary>('/analytics/summary', {
  immediate: true,
...
<template>
    <DemoWrapper title="TTL Cache" description="Cache hit returns data instantly without a network request. Expires after 15s." :code="code">
        ...
        <div v-if="data">{{ data.totalLists }} lists, {{ data.totalTasks }} tasks cached.</div>
